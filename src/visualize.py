...
# judge can flip between "satellite" and "map" view on the same page.
#
# The street layer USED to be CARTO Voyager (same free OSM road/place data,
# crisper style than folium's plain "OpenStreetMap" tiles). CARTO has since
# started requiring a (free, but email-signup-gated) API key for anonymous
# tile requests — unauthenticated requests now come back watermarked
# "API KEY REQUIRED" instead of a clean map. See templates/click_router.html
# and build_multi_city_map.py, which hit the exact same breakage, for the
# full writeup. Replacement: Esri's free, keyless ArcGIS Online "Canvas"
# gray-basemap tile service — same swap made in those two files.
#
# Esri's Canvas base tiles are a BARE gray canvas with no roads/labels baked
# in (unlike CARTO Voyager), so the base layer alone would render as a blank
# grid. A second tile layer (Canvas/World_Light_Gray_Reference, the matching
# roads/labels overlay) is added below as an always-on `overlay=True` layer
# stacked on top of whichever base layer (satellite or street) is active —
# folium's LayerControl has no direct equivalent of Leaflet's L.layerGroup
# for bundling two tile layers into one togglable base option, so this
# overlay approach is the folium-idiomatic way to get the same visual result.
#
# Honesty caveat carried over from the other two files: Esri's own service
# metadata only guarantees full worldwide tile coverage through zoom 13;
# "enhanced" coverage past that is documented for North America/Europe, not
# explicitly for India. max_native_zoom=13 (below) makes Leaflet stretch the
# last real zoom-13 tile for anything past that instead of requesting
# possibly-missing high-zoom tiles over India — never a blank/broken tile,
# but I have NOT visually verified how detailed those upscaled tiles look
# over Bengaluru specifically (only confirmed, via tile-fetch probes, that
# real images come back at zoom 13 and below).
...
STREET_TILES = "https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Light_Gray_Base/MapServer/tile/{z}/{y}/{x}"
STREET_LABELS_TILES = "https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Light_Gray_Reference/MapServer/tile/{z}/{y}/{x}"
STREET_ATTR = "Tiles &copy; Esri &mdash; Esri, DeLorme, NAVTEQ"
...
def render_route_map(Gc, waypoints: list[str], tour: list[int], out_path: str, title: str = "Route") -> None:
    ...
    folium.TileLayer(
        tiles=SATELLITE_TILES, attr=SATELLITE_ATTR, name="Satellite", overlay=False,
        max_zoom=19, max_native_zoom=19,
    ).add_to(m)
    folium.TileLayer(
        tiles=STREET_TILES, attr=STREET_ATTR, name="Street map", overlay=False,
        max_zoom=19, max_native_zoom=13, detect_retina=True,
    ).add_to(m)
    # Roads/labels for the street basemap — see the module-level comment on
    # STREET_LABELS_TILES for why this has to be a separate always-on overlay
    # rather than baked into the base layer above. Side effect, left in on
    # purpose rather than hidden: this overlay stays on when "Satellite" is
    # selected too, since folium's LayerControl can't sync an overlay's
    # visibility to which base layer is active without custom JS. Unlike
    # this file's satellite mode before this fix (which had NO label layer
    # at all), that's a net improvement, not a regression — dark road/place
    # labels drawn over imagery, roughly the same idea as click_router.html's
    # separate "Boundaries and Places" satellite-label layer, just reusing
    # one layer for both basemaps instead of a dedicated one per basemap.
    folium.TileLayer(
        tiles=STREET_LABELS_TILES, attr=STREET_ATTR, name="Street labels", overlay=True, show=True,
        control=False, max_zoom=19, max_native_zoom=13, detect_retina=True,
    ).add_to(m)
